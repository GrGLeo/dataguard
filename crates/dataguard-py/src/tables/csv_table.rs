use dataguard_core::{CsvTable as CoreCsvTable, Table as CoreTable};
...
/// Python wrapper for CsvTable from dataguard-core.
///
/// A CsvTable represents a CSV file to be validated with specific column rules.
#[pyclass(name = "CsvTable")]
pub struct CsvTable {
    pub(crate) inner: CoreCsvTable,
}

#[pymethods]
impl CsvTable {
    /// Create a new CsvTable instance.
    ///
    /// Args:
    ...
    ///
    /// Returns:
    ///     CsvTable: A new CsvTable instance.
    #[new]
    pub fn new(path: String, table_name: String) -> PyResult<Self> {
        let inner =
            CoreCsvTable::new(path, table_name).map_err(|e| PyIOError::new_err(e.to_string()))?;
        Ok(Self { inner })
    }
    ...
    /// Validate the CSV file against the prepared rules.
    ///
    /// Returns:
