...
pub(crate) struct UnicityAccumulator {
    ...
    accumulators: HashMap<String, (AtomicUsize, Arc<Mutex<HashSet<u64, Xxh3Builder>>>)>,
}

impl UnicityAccumulator {
    /// Create accumulator for columns that have unicity checks.
    pub fn new(columns: &[ExecutableColumn]) -> Self {
        let mut accumulators: HashMap<String, (AtomicUsize, Arc<Mutex<HashSet<u64, Xxh3Builder>>>)> =
            HashMap::new();
    ...
    pub fn record_hashes(&self, column_name: &str, null_count: usize, hashes: HashSet<u64, Xxh3Builder>) {
